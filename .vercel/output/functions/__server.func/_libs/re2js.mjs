...
var Compiler = class Compiler2 {
  ...
  rune(runes, flags) {
    ...
    if (runes.length !== 1 || Unicode.simpleFold(runes[0]) === runes[0]) flags &= -2;
    i.arg = flags;
    f.out = new PatchList(f.i << 1, f.i << 1);
  ...
  */
  ...
  push(re) {
    ...
    if (re.op === Regexp.Op.CHAR_CLASS && re.runes.length === 2 && re.runes[0] === re.runes[1]) {
      if (this.maybeConcat(re.runes[0], this.flags & -2)) return null;
...
var RE2JS = class RE2JS2 {
  ...
  */
  static compile(regex, flags = 0) {
    ...
    if ((flags & -544) !== 0) throw new RE2JSFlagsException("Flags should only be a combination of MULTILINE, DOTALL, CASE_INSENSITIVE, DISABLE_UNICODE_GROUPS, LONGEST_MATCH, LOOKBEHINDS");
    let re2Flags = RE2Flags.PERL;
    if ((flags & RE2JS2.DISABLE_UNICODE_GROUPS) !== 0) re2Flags &= -129;
